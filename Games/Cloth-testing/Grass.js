class Grass {
    ...
    begin() {
        ...
        var head = [];
        var tail = [];
        var particlesWidth = width / 2;
        this.wind = new Vec2D(0, 0);
        for (var i = 0; i < particlesWidth; i++) {
            var stepDir = new Vec2D(1, random(1, this.maxHeight));
            stringPart.push(new ParticleString2D(this.physics, new Vec2D(), stepDir, this.NUM_PARTICLES, -5, 1));
    ...
    update() {
        ...
        for (var i = 0; i < partLen - 1; i++) {
            ...
            if (!(i % this.NUM_PARTICLES))
                beginShape();
            var p1 = this.physics.particles[i];

                this.wind.set(random(0, -0.5), 0);
